...
class ConfigurationLoader {
    ...
    /// 加载默认工具配置
    private func loadDefaultTools() -> [ToolConfiguration] {
        // NPM
        let npm = ToolConfiguration(
            id: "npm",
            name: "NPM",
        ...
        let maven = ToolConfiguration(
            ...
            strategy: .xml(XMLStrategy(
                filePath: "~/.m2/settings.xml",
                set: XMLSetConfiguration(
                    xpath: "//mirrors/mirror[1]/url",
                    value: "{{url}}",
                    namespaces: nil,
                    ensureStructure: EnsureStructureConfiguration(
        ...
        // Homebrew
        let homebrew = ToolConfiguration(
            id: "homebrew",
            name: "Homebrew",
            ...
            strategy: .command(CommandStrategy(
                set: CommandSetConfiguration(
                    command: "git",
                    arguments: ["-C", "{{brew:prefix}}", "remote", "set-url", "origin", "{{url}}"],
                    environment: nil,
                    requiresAdmin: false,
                    workingDirectory: nil,
                    preCommands: [
                ...
                get: CommandGetConfiguration(
                    command: "git",
                    arguments: ["-C", "{{brew:prefix}}", "remote", "-v"],
        ...
        // OrbStack
        let orbstack = ToolConfiguration(
            id: "orbstack",
            name: "OrbStack",
            ...
            detection: DetectionConfiguration(
                ...
                arguments: ["--version"],
                customPaths: nil,
                fallbackDetection: FallbackDetection.file(path: "/Applications/OrbStack.app")
            ),
            sources: [
                SourceConfiguration(
            ...
            strategy: .jsonpath(JSONPathStrategy(
                filePath: "~/.orbstack/config/docker.json",
                set: JSONPathSetConfiguration(
                    jsonpath: "$.registry-mirrors",
                    value: .array([.string("{{url}}")]),
        ...
        return [npm, maven, homebrew, orbstack]
    }
